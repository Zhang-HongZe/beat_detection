...
{
    float bin_hz = (float)handle->sample_rate / (float)handle->fft_size;
    int bin = (int)roundf(hz / bin_hz);
    if (bin < 1) {
        return 1;
    }
    if (bin >= handle->fft_size / 2) {
        return (handle->fft_size / 2) - 1;
    }
    return (uint16_t)bin;
}
...
esp_err_t beat_detection_data_write(beat_detection_handle_t handle, beat_detection_audio_buffer_t buffer)
{
    ...
    if (buffer.bytes_size < handle->channel * handle->fft_size * sizeof(int16_t)) {
    ...
    uint8_t *audio_buffer = (uint8_t *)heap_caps_malloc(handle->channel * handle->fft_size * sizeof(int16_t), loc | MALLOC_CAP_8BIT);
    if (audio_buffer == NULL) {
        ESP_LOGE(TAG, "Failed to allocate memory for audio buffer");
        return ESP_ERR_NO_MEM;
    }
    memcpy(audio_buffer, buffer.audio_buffer, handle->channel * handle->fft_size * sizeof(int16_t));

    beat_detection_audio_buffer_t processed_buffer = {
        .audio_buffer = audio_buffer,
        .bytes_size = handle->channel * handle->fft_size * sizeof(int16_t),
    };
    if (xQueueSend(handle->audio_queue, &processed_buffer, pdMS_TO_TICKS(0)) != pdPASS) {
...
static void beat_detection_task(void *arg)
{
    ...
    while (true) {
        ...
        xQueueReceive(handle->audio_queue, &received_buffer, portMAX_DELAY);
        handle->status.is_calculating = true;
        if (received_buffer.bytes_size < handle->channel * handle->fft_size * sizeof(int16_t)) {
            ESP_LOGE(TAG, "Audio buffer size is less than FFT size");
            continue;
        }
...
esp_err_t beat_detection_init(beat_detection_cfg_t *cfg, beat_detection_handle_t *handle)
{
    ...
    *handle = (beat_detection_handle_t)heap_caps_malloc(sizeof(beat_detection_t), local_flags | MALLOC_CAP_8BIT);
    ...
    memset((*handle)->fft_buffer, 0, 2 * (*handle)->fft_size * sizeof(float));

    (*handle)->window = heap_caps_malloc((*handle)->fft_size * sizeof(float), local_flags | MALLOC_CAP_8BIT);
    if ((*handle)->window == NULL) {
        ESP_LOGE(TAG, "Failed to allocate memory for window");
        beat_detection_deinit(handle);
        return ESP_ERR_NO_MEM;
    }
    memset((*handle)->window, 0, (*handle)->fft_size * sizeof(float));
    dsps_wind_hann_f32((*handle)->window, (*handle)->fft_size);

    (*handle)->magnitude = (float *)heap_caps_malloc((*handle)->fft_size / 2 * sizeof(float), local_flags | MALLOC_CAP_8BIT);
    if ((*handle)->magnitude == NULL) {
        ESP_LOGE(TAG, "Failed to allocate memory for magnitude");
        beat_detection_deinit(handle);
        return ESP_ERR_NO_MEM;
    }
    memset((*handle)->magnitude, 0, (*handle)->fft_size / 2 * sizeof(float));

    (*handle)->magnitude_prev = (float *)heap_caps_malloc((*handle)->fft_size / 2 * sizeof(float), local_flags | MALLOC_CAP_8BIT);
    if ((*handle)->magnitude_prev == NULL) {
        ESP_LOGE(TAG, "Failed to allocate memory for magnitude previous");
        beat_detection_deinit(handle);
        return ESP_ERR_NO_MEM;
    }
    memset((*handle)->magnitude_prev, 0, (*handle)->fft_size / 2 * sizeof(float));

    (*handle)->audio_queue = xQueueCreate(1, sizeof(beat_detection_audio_buffer_t));
    ...
    if (cfg->flags.enable_psram) {
        ...
        memset((*handle)->task_tcb, 0, sizeof(StaticTask_t));
...
static inline bool detect_bass_surge(float current_bass, float prev_bass, float threshold)
{
    if (prev_bass <= 0.0f) {
        return false;
    }
...
beat_detection_result_t beat_detection(beat_detection_handle_t handle, int16_t *audio_buffer)
{
    ...
    if (handle->channel == 1) {
        for (int i = 0; i < handle->fft_size; i++) {
            handle->fft_buffer[2 * i] = ((float)(audio_buffer[i]) / 32768.0f) * handle->window[i];
            handle->fft_buffer[2 * i + 1] = 0.0f;
        }
    } else if (handle->channel == 2) {
        for (int i = 0; i < handle->fft_size; i++) {
            handle->fft_buffer[2 * i] = ((float)(audio_buffer[2 * i + 1]) / 32768.0f) * handle->window[i];
            handle->fft_buffer[2 * i + 1] = 0.0f;
        }
    } else {
    ...
    dsps_fft2r_fc32(handle->fft_buffer, handle->fft_size);
    dsps_bit_rev_fc32(handle->fft_buffer, handle->fft_size);

    for (int i = 0; i < handle->fft_size / 2; ++i) {
        float real = handle->fft_buffer[2 * i];
        float imag = handle->fft_buffer[2 * i + 1];
        float a = 0.9f;
        float magnitude = sqrtf(real * real + imag * imag);
        handle->magnitude[i] = a * magnitude + (1 - a) * handle->magnitude_prev[i];
    }
    ...
    for (int bin = handle->bass_bin_start; bin <= handle->bass_bin_end && bin < handle->fft_size / 2; ++bin) {
        if (handle->magnitude_prev[bin] > prev_bass) {
            prev_bass = handle->magnitude_prev[bin];
        }
        if (handle->magnitude[bin] > current_bass) {
            current_bass = handle->magnitude[bin];
        }
        current_sum += handle->magnitude[bin];
        prev_sum += handle->magnitude_prev[bin];
    }
    average_ratio = current_sum / prev_sum;

    memcpy(handle->magnitude_prev, handle->magnitude, sizeof(float) * handle->fft_size / 2);

    // detect bass drum
    if ((detect_bass_surge(current_bass, prev_bass, handle->bass_surge_threshold) || average_ratio > 5.0f) 
        && current_bass > 0.01f 
        && xTaskGetTickCount() - handle->last_bass_detected_time > pdMS_TO_TICKS(100)) {
        ...
        handle->last_bass_detected_time = xTaskGetTickCount();
...
esp_err_t beat_detection_deinit(beat_detection_handle_t *handle)
{
    ...
    if ((*handle)->magnitude != NULL) {
        heap_caps_free((*handle)->magnitude);
    }
    if ((*handle)->magnitude_prev != NULL) {
        heap_caps_free((*handle)->magnitude_prev);
    }
    if ((*handle)->audio_queue != NULL) {
        vQueueDelete((*handle)->audio_queue);
    }
    if ((*handle)->task_stack_buffer != NULL) {
        heap_caps_free((*handle)->task_stack_buffer);
    }
    if ((*handle)->task_tcb != NULL) {
        heap_caps_free((*handle)->task_tcb);
    }
    if ((*handle)->task_handle != NULL) {
